...
namespace phoenix
{
	...
	static const float NEAR_PLANE = 1.0f, FAR_PLANE = 7.5f, ORTHO_PROJ_HALF_WIDTH = 10.0f;
	...
	static const float AMBIENT_FACTOR = 0.3f, SPECULAR_FACTOR = 64.0f, CALIBRATED_LIGHT_SIZE = 30.0f;
	...
	public:
		...
		unsigned int _renderMode = 0;
		unsigned int _floorTexture = 0;
		unsigned int _objectTexture = 0;
		unsigned int _altObjTexture = 0;

		ShadowCommon(glm::vec3 lightPos) : _lightPos(lightPos) {}
		...
		void setUniforms(const phoenix::Shader&, const Camera*);
		void renderDebugLines(const phoenix::Shader&, Utils*);
		void setLightSpaceVP(const phoenix::Shader& shader);

	private:
		unsigned int _debugLinesVAO = 0;
		unsigned int _debugLinesVBO = 0;
	};
}